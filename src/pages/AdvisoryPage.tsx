...
// Hook to fetch sub-executive members
const useSubExecutiveMembers = () => {
  return useQuery({
    queryKey: ['sub-executive-members'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('sub_executive_members')
        .select('*')
        .eq('active', true)
...
const AdvisoryPage = () => {
  const { data: advisoryMembers, isLoading, error } = useSubExecutiveMembers();
      ...
      <PageLayout>
        <div className="container mx-auto px-4 py-16 min-h-screen">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-5xl font-bold text-white mb-12 text-center">Sub-Executive</h1>
    ...
    return (
      <PageLayout>
        <div className="container mx-auto px-4 py-16 min-h-screen">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-5xl font-bold text-white mb-12 text-center">Sub-Executive</h1>
            <div className="text-center text-red-500">Error loading sub-executive members</div>
          </div>
        </div>
      </PageLayout>
    );
              ...
              <Card key={member.id} className="bg-black/60 border-cyan-400/30 hover:border-cyan-400/60 transition-all duration-300 overflow-hidden group">
                <div className={`relative h-64 bg-gradient-to-br ${gradients[index % gradients.length]}`}>
                  ...
                  <div className="absolute bottom-4 left-4 text-white">
                    ...
                    <p className="text-cyan-300 font-medium">{member.title}</p>
                  </div>
